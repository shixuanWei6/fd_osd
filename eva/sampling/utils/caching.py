...
def prune_dynamic_cache(cache: DynamicCache, num_tokens_to_discard: int):
    """
    Prune the cache by removing the specified number of tokens from the end.
    Works with the new DynamicCache API in HuggingFace Transformers.
    """
    if cache is None:
        return None
    ...
    for layer in cache.layers:
        if hasattr(layer, "key") and layer.key is not None:
            layer.key = layer.key[:, :, :-num_tokens_to_discard, :]
        if hasattr(layer, "value") and layer.value is not None:
            layer.value = layer.value[:, :, :-num_tokens_to_discard, :]
