...
from training.trainer import set_trainable_adapters
...
def _sync_if_cuda(model):
    ...
    # `speculative_inference_with_feedback` may dispatch across GPUs under `device_map="auto"`,
    # and `torch.cuda.synchronize()` is the most reliable barrier.
    if torch.cuda.is_available():
...
def run_static_speculative_for_prompt(
    ...
    *,
    ...
    # Free the (potentially large) tensor promptly.
    del generated_ids
...
def main():
    ...
    exp_suffix = os.environ.get("EXP_NAME", f"r{args.r}_static_baseline_multiclient")
    if args.debug:
        exp_suffix += "_debug"
    ...
    print(f"\n=== Setting up {len(domains)} Client Draft Models (static, no training) ===")
    clients = []
    for client_id, domain in enumerate(domains):
        ...
        c_draft = setup_dual_lora(c_draft, r=args.r, lora_alpha=args.lora_alpha)
        # Mirror `main_pre_exp.py` trainable adapter selection (important for init parity).
    ...
    print("\n=== Running static speculative inference (no online training) ===")
    ...
    for step, prompt_tuple in enumerate(pbar):
        ...
        step_model = clients[0]["model"]
        _sync_if_cuda(step_model)
    ...
    # -----------------------
    print("\n" + "=" * 70)
    print("STATIC BASELINE QUANTITATIVE RESULTS SUMMARY")
    print("=" * 70)
    for i, domain in enumerate(domains):
        ...
        print(f"{domain:<8} | Mean Alpha: {mean_alpha:.4f} | Mean Latency: {mean_lat_ms:.2f} ms | p90: {p90_lat_ms:.2f} ms")
    ...
    plt.title("Static Speculative Decoding Baseline: Alpha vs Step")
    ...
    plt.title("Static Speculative Decoding Baseline: Latency vs Step")
    ...
    plt.title("Static Speculative Decoding Baseline: Total Step Latency")
    plt.legend(loc="upper right")
    plt.grid(True, linestyle=":", alpha=0.6)
    plt.tight_layout()
